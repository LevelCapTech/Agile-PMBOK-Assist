...
`main` ブランチへの push で `.github/workflows/deploy-gh-pages.yml` が実行され、`mock/v1/web` のビルド成果物が GitHub Pages に公開されます。手動実行する場合は、GitHub Actions の `Deploy React App to GitHub Pages` を選び、必要であれば `mock_version` を指定して実行してください。
