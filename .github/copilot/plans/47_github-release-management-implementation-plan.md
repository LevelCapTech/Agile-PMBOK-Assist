...
- 責務分離 / データフロー:
  ...
  - Next.js standalone bundle Release asset 登録は日付ベース Release 管理と同一 workflow で実行し、以下の順序で処理する。既存Release確認はタグ算出後に実施し、build前に fail-fast とする。
    ...
    3. npm ci（リポジトリルート）
    4. `release_date`（JST 日付）を基にタグ計算: `YYYY.MM.DD` / `YYYY.MM.DD-2` / `YYYY.MM.DD-3` ...
...
- エッジケース / 例外系 / リトライ方針:
  ...
  - レースコンディションの一次対策として、workflow は同一日付/タグの並列実行を concurrency で抑制し、採用予定タグ/同名 Release 確認を build 前に実施する。concurrency.group は `workflow_dispatch` の input で受け取る日付（例: `release_date`）を用いて `release-date-${{ github.event.inputs.release_date }}` のように「workflow 開始時点で決まる値」で定義し、ステップ内で計算した JST 日付は concurrency group には用いない。
  - 採用予定タグ（同名の Git tag）または同名 Release が既に存在する場合は workflow を失敗させる（レースコンディションの最終ガード）。
  ...
  - `gh release view <tag>` または GitHub API で同名 Release を検知した場合は workflow を失敗させる。
...
- テスト観点（正常 / 例外 / 境界 / 回帰）:
  ...
  - 境界: 同日 3 回目の実行で `-3` が付いたタグが生成される。
  - 境界: `YYYY.MM.DD` と `YYYY.MM.DD-5` が存在する場合に `-2` が採用される。
  - 回帰: 同日既存タグ一覧から採用予定タグを算出しても同名 Release が重複作成されない。
  - 正常: workflow_dispatch 実行で `next-bundle.tgz` が Release に登録される。
  - 例外: build 失敗時は Release が作成されない。
  - 例外: 採用予定タグ（同名の Git tag）が存在する場合に workflow が失敗する。
  - 境界: 同名 Release が存在する場合に workflow が失敗する。
  - 回帰: bundle 内に `server.js` と `.next/static` が含まれる。
  - 例外: 同名 Release が存在する場合に asset 追記・上書きを行わず fail する。
...
- ロールバック方法:
  ...
  - 誤った bundle asset が登録された場合は Release とタグを削除して再実行する、または新しい枝番タグ（例: `YYYY.MM.DD-2` / `YYYY.MM.DD-3`）で再リリースする。
- 監視・運用上の注意:
  ...
  - 既存 Release への asset 追記は禁止のため、過去リリース素材を修正する場合は新しい枝番タグで再リリースする。
