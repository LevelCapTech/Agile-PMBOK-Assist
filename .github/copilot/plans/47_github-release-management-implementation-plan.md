...
- エッジケース / 例外系 / リトライ方針:
  ...
  - レースコンディションの一次対策として、workflow は同一日付/タグの並列実行を concurrency で抑制し、採用予定タグ/同名 Release 確認を build 前に実施する。concurrency group は `release-date-${YYYY.MM.DD}` のように日付を含めて定義する。
