...
- DI 起点は Next.js のエントリに固定する（Pages Router: `pages/_app.tsx`、App Router: `app/layout.tsx`）。
- `page.tsx` / `pages/*.tsx` は DI コンテナを生成せず、`AppContext` から依存を受け取り UI へ渡す薄いラッパーとして実装する。
...
- cookie/session の読取は Server 実行境界（`getServerSideProps` / Route Handler / Server Component）でのみ行う。
- public 完成実装の差し替え点は `packages/plugins` に集約する。
- `app/` / `src/` / `pages/` では `index.ts` 経由の再エクスポート import を禁止し、定義ファイルを直接参照する。
