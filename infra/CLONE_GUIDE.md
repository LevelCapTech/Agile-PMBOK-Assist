...
6. 「Generate a private key」で PEM を発行し、VPS に配置する（GitHub やリポジトリには置かず、Git 管理外にし、誤登録防止のため `.gitignore` にも記載する）。
...
- `agile-pmbok-assist-pull.timer` による自動 pull は `reset --hard` で同期するため、未コミットのローカル変更は破棄されます（未追跡ファイルは残ります）。
